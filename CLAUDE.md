...
- **Sidebar principal (`nav.js`)** — árbol de dos niveles de despliegue, todo dentro del mismo `#sb-nav`:
  ...
  - `_renderNavExpandable(nav, item)` pinta la entrada raíz como `.nav-it` con chevron (`.ni-chev`), controlada por `_navAdminOpen` (forzado a `true` mientras `CUR_PAGE === 'admin'`, así la sección activa nunca se puede ocultar accidentalmente).
  - Si está abierta, por cada grupo visible agrega un header `.nav-subsec` (clickeable, con su propio chevron `.ns-chev`) — el grupo que contiene la subsección activa se auto-expande y no se puede colapsar mientras siga activo (`_navAdminGruposAbiertos`, misma lógica que `_navAdminOpen` pero por grupo). Los demás grupos arrancan colapsados y se expanden/colapsan libremente al clic, sin afectarse entre sí (no es acordeón exclusivo).
